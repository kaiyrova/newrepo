...
def PreprocessData_Instruments(filname, DatasetName, sep="\t"):
    col_names = ["user", "item", "ts"]
    df = pd.read_csv(filname, sep=sep, header=None, names=col_names, engine='python')
    for col in ("user", "item"):
        df[col] = df[col].astype(np.int32)
...
def get_ItemDataInstruments(itemnum):
    ItemFeatures = load_data('./Data/Musical_Instruments_feat.dat')
    ItemFeatures = np.vstack((np.zeros(ItemFeatures.shape[1]), ItemFeatures))
    return ItemFeatures
...
if dataset_name == 'Musical_Instruments':
    class Args:
...
if dataset_name == 'Musical_Instruments' :
    TrainDf, CXTDict = PreprocessData_Instruments("./Data/Musical_Instruments_cxt.txt", args.dataset, sep=" ")
    save_data(CXTDict,'./Data/CXTDictSasRec_Instruments.dat')
...
if args.dataset == 'Musical_Instruments' :
  ItemFeatures = get_ItemDataInstruments(itemnum)
  #UserFeatures = get_UserDataFashion(usernum)
  UserFeatures = []
  CXTDict = load_data('./Data/CXTDictSasRec_Instruments.dat')
